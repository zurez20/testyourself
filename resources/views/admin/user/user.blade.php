...
<div class="card">
    <div class="card-body p-0">
        ...
        <div class="card-px text-center py-20 ">
            ...
            @if (in_array('add-user-user', config('addPermissions')))
    ...
    <div class="card-header border-0 pt-6">
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <input type="text" data-kt-customer-table-filter="search" class="form-control w-250px ps-15" placeholder="Search User" />
            </div>
        </div>
        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5" id="data_table">
            <thead>
                <tr class="text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                    ...
                    <th class="text-center min-w-25px">Employee No</th>
                    ...
                    @if (in_array('update-user-user', config('updatePermissions')) || in_array('delete-user-user',config('deletePermissions')))
            ...
            <tbody class="fw-semibold text-gray-600">
                ...
                <tr>
                    ...
                    <td class="text-center">
                        {{$data->rolee ? $data->rolee->name : 'Not Found'}}<br>
                        @if (in_array('update-user-user', config('updatePermissions')))
                        @if($data->rolee->panelFlag == 1)
                        <a href="{{url('/admin/user/permissions?userUID='.$data->uid)}}"
                            class="badge badge-primary" style="text-decoration: none; color: white;">View Permissions
                        </a>
                        @else
                        <span class="badge badge-light-danger">No panel access for this role </span>
                        @endif
                        @endif
                    </td>
                    <td class="text-center">
                    ...
                    <td class="text-center">
                        <div class="align-middle text-center">
                            @if (in_array('update-user-user', config('updatePermissions')))
                            <a class="btn btn-icon btn-outline-warning has-ripple" href="{{'admin/user/update?userUid='.$data->uid}}" style="border-radius: 50%;"><i class="fas fa-pen"></i></a>
                            @endif
                            @if (in_array('delete-user-user',config('deletePermissions')))
    ...
    <div class="modal-dialog ">
        <div class="modal-content">
            ...
            <form action="{{url('admin/user/delete')}}" id="deleteForm" method="post">
                @csrf
                <input type="hidden" name="userUid" id="deleteUserUid">
                <div class="modal-body">
                    <span>Are you sure you want to delete user <span id="userName"></span> ? <br> Action cannot be reverted.</span>
                </div>
...
<script>
    ...
    function openDeleteModal(uid) {
        var user = users.find(x => x.uid == uid);
        $('#deleteUserUid').val(user.uid);
